...
| Vault address | From your Vault operator, Vault Agent config, or local `VAULT_ADDR`. Local development often uses `http://127.0.0.1:8200`. | Copy the Vault server URL, not a UI page URL. |
| Vault token file | From Vault Agent token sink, platform secret mount, or local `vault login`. Local development commonly uses `~/.vault-token`. | Copy the filesystem path readable by the account running the MCP. |
| Vault API key ref | Usually generated by `limacharlie-mcp-configure`. The default is `vault://secret/data/limacharlie/mcp#api_key`. | Copy only if the key is already in Vault and you are using `--skip-vault-write`. |
...
- `vault-token-file.png`: A generic Vault Agent token sink or local `vault login` token-file example, with token values redacted.
...
4. Run `limacharlie-mcp-configure` with the org ID, Vault address, and Vault
   token file. The helper reads the LimaCharlie key through a hidden prompt,
   writes it to Vault KV v2, and writes a nonsecret config file.
...
```json
{
  ...
  "vault_token_file": "/Users/you/.vault-token"
}
```
